exports.handler = async (event) => {
  ...
  const unit_measure = event.queryStringParameters && event.queryStringParameters.unit_measure;

  let url = `https://data360api.worldbank.org/data360/data?DATABASE_ID=IMF_FAS&INDICATOR=IMF_FAS_FCCCC&REF_AREA=${country}&timePeriodFrom=${start_year}&timePeriodTo=${end_year}&skip=0`;
  if (unit_measure) {
    url += `&UNIT_MEASURE=${encodeURIComponent(unit_measure)}`;
  }
